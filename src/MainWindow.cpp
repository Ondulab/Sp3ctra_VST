...
// Inclure les en-têtes C++ pour MIDI et RtAudio
...
MainWindow::MainWindow(QWidget *parent)
  ...
  sfRenderWindow *window =
      sfRenderWindow_create(mode, "CISYNTH", sfResize | sfClose, NULL);
  if (!window) {
    QMessageBox::critical(this, "Erreur",
                          "Impossible de créer la fenêtre SFML");
    return;
  }
...
void MainWindow::initializeAudio() {
  ...
  // Connecter le contrôleur MIDI et configurer le callback de volume
  if (gMidiController && gAudioSystem) {
    // Définir le callback pour le contrôle du volume
    gMidiController->setVolumeChangeCallback([this](float volume) {
      // Mettre à jour le volume principal dans le système audio
      if (gAudioSystem) {
        gAudioSystem->setMasterVolume(volume);

        // Format du message avec pourcentage pour meilleure lisibilité
        char volumeMessage[50];
        snprintf(volumeMessage, sizeof(volumeMessage), "Volume: %d%%",
                 (int)(volume * 100));

        // Mettre à jour la barre d'état avec le niveau de volume actuel
        m_statusLabel->setText(volumeMessage);

        // Afficher dans la console pour débogage
#ifdef DEBUG_MIDI
        std::cout << "MIDI Volume change: " << volumeMessage << std::endl;
#else
        printf("Volume principal: %.2f (%s)\n", volume, volumeMessage);
#endif
      }
    });
    ...
    if (midi_Connect()) {
      statusBar()->showMessage("MIDI: Launchkey Mini connecté", 3000);
    } else {
      QStringList midiDevices;
      auto devices = gMidiController->getAvailableDevices();
      for (const auto &device : devices) {
        midiDevices << QString::fromStdString(device);
      }

      if (midiDevices.isEmpty()) {
        statusBar()->showMessage("MIDI: Aucun périphérique trouvé", 3000);
      } else {
        statusBar()->showMessage(
            "MIDI: Périphériques disponibles, mais Launchkey Mini non trouvé",
            3000);
      }
    }
  }
...
void MainWindow::initializeDMX() {
  ...
  // Initialiser DMX
  int dmxFd =
      init_Dmx("/dev/tty.usbserial",
               0); // Utiliser le port par défaut et ne pas être silencieux
  if (dmxFd < 0) {

...
Delay::Delay(void) : _size(0), _line(0) {}

Delay::~Delay(void) { fini(); }
...
void Filt1::set_params(float del, float tmf, float tlo, float wlo, float thi,
                       float chi) {
  float g, t;
...
void Reverb::init(float fsamp, bool ambis) {
  ...
  _ipdel = DEFAULT_REVERB_PREDELAY;
  _xover = DEFAULT_REVERB_XOVER;
  _rtlow = DEFAULT_REVERB_RT_LOW;
  _rtmid = DEFAULT_REVERB_RT_MID;
  _fdamp = DEFAULT_REVERB_FDAMP;
  _opmix = DEFAULT_REVERB_OPMIX;
  _rgxyz = DEFAULT_REVERB_RGXYZ;

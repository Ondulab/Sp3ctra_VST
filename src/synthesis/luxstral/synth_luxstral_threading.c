...
void synth_precompute_wave_data(float *imageData, DoubleBuffer *db) {
  ...
  for (int i = 0; i < num_workers; i++) {
    ...
    for (int note = worker->start_note; note < worker->end_note; note++) {
      ...
      //
      // Table is generated at the REFERENCE octave (WAVE_REF_OCTAVE) so its
      // virtual sample rate is Fs × 2^R.  phase_inc = f_note / f_ref_comma:
      ...
      //
      // Linear interpolation between i0 and i1 eliminates the staircase
      // distortion that the previous integer-step approach produced at high
      // octaves, improving THD by ~50-60 dB on the highest notes.
      //
      // Workload per note is constant (audio_buffer_size iterations + 1 lerp
      // each) regardless of octave, so note-count-based worker partitioning
      // remains the optimal load-balancing strategy.
      //
      ...
      // Commit phase continuity: safe because workers are still blocked at
      // g_worker_start_barrier and next precompute call happens only after
      // all workers pass g_worker_end_barrier (next iteration).

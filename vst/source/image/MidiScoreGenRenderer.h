 ...
 * Y = pitch on the LOG frequency axis, ink = velocity). Two upgrades over
 * the Python script:
...
/** Seconds of music one A4 page band holds at these settings. */
double pageSeconds(const MidiScoreSettings& s);

/** Number of A4 pages needed for the whole piece (≥ 1 when it has notes). */
...
/** Renders one A4 portrait export page (pageIndex ∈ [0..pageCount-1]): white
 *  page, band at the SCORE geometry, opt-in title/footer in the top margin.
 *  Notes crossing a page boundary continue on the next page with their
 *  envelope phase preserved. */

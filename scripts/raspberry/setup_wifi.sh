#!/usr/bin/env bash
# setup_wifi.sh - Configure WiFi interface on Raspberry Pi (Debian/Bookworm)
# - Sets up WiFi with provided credentials
# - Manages WiFi band selection and routing priority
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -h|--help)
                echo "Usage: sudo $0 --ssid \"WIFI_NAME\" --psk \"WIFI_PASSWORD\" [OPTIONS]"
                echo ""
                ...
                echo "  --interface IFACE WiFi interface name (default: wlan0)"
                echo "  --metric METRIC   Routing metric (default: 200)"
                exit 0 ;;
            *)
...
configure_networkmanager_security() {
    log "Configuring NetworkManager security settings for WiFi"
...
configure_wifi() {
    ...
    log "WiFi configured for SSID: ${SSID} with metric ${WIFI_METRIC}"
...
activate_wifi() {
    log "Activating WiFi connection"
    ...
    while [[ ${retry_count} -lt ${max_retries} && "${wifi_success}" == "false" ]]; do
        ...
        if nmcli connection up "${WIFI_CONN}" ifname "${WIFI_IFACE}" 2>/dev/null; then
            ...
            if [[ ${retry_count} -lt ${max_retries} ]]; then
                ...
                fail "Failed to activate WiFi after ${max_retries} attempts. Check SSID availability and password correctness"
            fi
        fi
    done
...
show_wifi_status() {
    echo
    log "WiFi device status:"
    nmcli device status | grep -E "(DEVICE|${WIFI_IFACE})" || true
    
    echo
    log "WiFi connection status:"
    nmcli connection show --active | grep -E "(NAME|${WIFI_CONN})" || true
    
    echo
    log "WiFi IP configuration:"
    ip addr show "${WIFI_IFACE}" 2>/dev/null || true
    
    echo
    log "Routing table (WiFi routes):"
    ip route | grep "${WIFI_IFACE}" || true
}
...
main() {
    ...
    log "WiFi configuration completed. Changes will persist after reboot."
}
